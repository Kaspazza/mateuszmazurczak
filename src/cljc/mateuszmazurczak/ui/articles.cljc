       ...
       (let [scroll-y (.-scrollY js/window)
         ...
         (if (< scroll-y 50)
           ...
           (let [positions (for [{:keys [id]} headings
                 ...
                 visible-in-viewport (filter #(and (>= (:top %) 0)
                                                   (<= (:top %)
                                                       (/ viewport-height 2)))
                                             positions)
                 just-above (filter #(and (< (:distance %) 0)
                                          (> (:distance %) -300))
                                    positions)
                 best-match
                 (cond
                   (seq just-above)
                   (first (sort-by (juxt #(Math/abs (:distance %)) :index)
                                   just-above))
                   (seq visible-in-viewport)
                   (first (sort-by :top visible-in-viewport))
                   :else (first (sort-by (juxt #(Math/abs (:distance %)) :index)
                                         positions)))]
...
#?(:cljs (defn heading-change
           ...
           [active-id headings]
           (let [id-to-index (into {}
                                   (map-indexed (fn [idx {:keys [id]}] [id idx])
                                                headings))]
             (throttle-header-change
              #(determine-active-heading active-id headings id-to-index)
              50))))
