...
(defn breadcrumb
  ...
  (into [:nav
         (-> props
          ...
          (dissoc :class-name :separator))] children))
...
(defn breadcrumb-list
  ...
  (into
   [:ol
    (-> props
     (assoc :data-slot "breadcrumb-list" :class
      (merge-classes
...
(defn breadcrumb-item
  ...
  (into [:li
         (-> props
          (assoc :data-slot "breadcrumb-item" :class (merge-classes
                                                      "inline-flex items-center gap-1.5" class))
          (dissoc :class-name))] children))
...
(defn breadcrumb-link
  ...
  (let [component (if as-child Slot "a")]
    (into [:>
           component
           (-> props
            (assoc :data-slot "breadcrumb-link" :class
             (merge-classes "transition-colors hover:text-foreground" class))
            (dissoc :class-name :as-child))] children)))
...
(defn breadcrumb-page
  ...
  (into [:span
         (-> props
          (assoc :role "link" :aria-disabled "true" :aria-current "page" :data-slot
           "breadcrumb-page" :class (merge-classes "font-normal text-foreground" class))
          (dissoc :class-name))] children))
