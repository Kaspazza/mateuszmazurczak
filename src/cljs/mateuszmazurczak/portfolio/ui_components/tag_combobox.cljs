...
(defscene tag-with-custom-placeholder
  ...
  (let [tags (r/atom #{"feature" "bug" "enhancement" "documentation"})
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Issue Type"]
        [sut/tag-combobox
         {:tags @tags
...
(defscene tag-with-custom-width
  ...
  (let [tags (r/atom #{"red" "green" "blue" "yellow" "purple"})
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Color Tag"]
        [sut/tag-combobox
         {:tags @tags
          :selected-tag @selected
          :class "w-[200px]"
...
(defscene multiple-tag-comboboxes
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8 space-y-6"}
        [:div
         [:h3 {:class "text-lg font-semibold mb-2"} "Task Management"]
        ...
        [:div {:class "mt-6 p-4 bg-muted rounded-md"}
         [:p {:class "text-sm font-medium"} "Selection:"]
         [:p {:class "text-xs mt-2"}
          (str "Priority: " @selected-priority)]
         [:p {:class "text-xs"}
          (str "Status: " @selected-status)]]]))))

(defscene tag-creation-workflow
  ...
  (let [tags (r/atom #{"existing-tag-1" "existing-tag-2"})
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8 space-y-4"}
        [:div
         [:h3 {:class "text-lg font-semibold mb-2"} "Tag Creation Demo"]
         [:p {:class "text-sm text-muted-foreground mb-4"}
        ...
        [:div {:class "mt-6 p-4 bg-muted rounded-md"}
         [:p {:class "text-sm font-medium mb-2"} "Available tags:"]
         ...
         (when (seq @creation-log)
           [:div
            [:p {:class "text-sm font-medium mb-2"} "Creation log:"]
            [:div {:class "space-y-1"}
