...
(defn ^:export init!
  ...
  (try (error-tracking/init-error-tracking!
        {:dsn (web-conf/read-param [:log :sentry :frontend :dsn])
         :traced-website #"^https://hephaistox\.com/"
         :env (web-conf/read-param [:env])})
       (core-log/info "Front end starting")
       (web-events-proxy/client-app-db-init! ::mateuszmazurczak-fe-events/initialize-db) ;; What is done before will be lost in the state
       (mount-root)
       (rt/start-router!)
       (mount/start)
