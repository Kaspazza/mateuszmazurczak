...
(defc carousel
 ...
 (let [use-embla-carousel (or (gobj/get embla-carousel "default") embla-carousel)
       ...
       handle-key-down (rhooks/use-callback
                        (fn [event]
                          (when (= (.-key event) "ArrowLeft") (.preventDefault event) (scroll-prev))
                          (when (= (.-key event) "ArrowRight")
                            (.preventDefault event)
                            (scroll-next))) [scroll-prev scroll-next])]
   ;; Set API callback
   ...
   (rhooks/use-effect (fn []
                        (when api
                          ...
                          ;; Cleanup function
                          (fn [] (.off api "select" on-select)))) [api on-select])
   ;; Update context
   (reset! carousel-context
     {:carousel-ref carousel-ref
      :api api
      :opts opts
      :orientation (or orientation (if (= (:axis opts) "y") :vertical :horizontal))
...
(defn carousel-content
  ...
  (let [{:keys [carousel-ref orientation]} (use-carousel)]
    [:div {:ref carousel-ref
     ...
     (into [:div
            (-> props
             (assoc :class (merge-classes "flex"
                            (if (= orientation :horizontal) "-ml-4" "-mt-4 flex-col") class)
              :data-slot "carousel-content-inner")
             (dissoc :class))] children)]))
...
(defn carousel-item
  ...
  (let [{:keys [orientation]} (use-carousel)]
    (into [:div
           (-> props
            (assoc :role "group" :aria-roledescription "slide" :data-slot "carousel-item" :class
             (merge-classes "min-w-0 shrink-0 grow-0 basis-full"
              (if (= orientation :horizontal) "pl-4" "pt-4") class))
            (dissoc :class))] children)))
      ...
      (assoc
       ...
       :class (merge-classes "absolute size-8 rounded-full"
               (if (= orientation :horizontal)
                 ...
                 "-top-12 left-1/2 -translate-x-1/2 rotate-90") class)
      ...
      (assoc
       ...
       :class (merge-classes "absolute size-8 rounded-full"
               (if (= orientation :horizontal)
                 ...
                 "-bottom-12 left-1/2 -translate-x-1/2 rotate-90") class)
