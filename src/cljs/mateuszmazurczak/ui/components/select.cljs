...
(defn select-trigger
  ...
  (into [:>
         RadixSelect/Trigger
         (-> props
          (assoc :data-slot "select-trigger" :data-size size :class
           (merge-classes ["border-input data-[placeholder]:text-muted-foreground"
   ...
   (concat children [[:> RadixSelect/Icon {:asChild true} (r/as-element [chevron-down-icon])]])))
  ...
  [:>
   ...
   (into [:>
          ...
          {:data-slot "select-content"
           :position position
           :align align
           :class (merge-classes "bg-popover text-popover-foreground" "data-[state=open]:animate-in"
                   ...
                   (when (= position "popper")
                     (merge-classes "data-[side=bottom]:translate-y-1"
                      "data-[side=left]:-translate-x-1" "data-[side=right]:translate-x-1"
                      "data-[side=top]:-translate-y-1")) class)}]
    (concat [[select-scroll-up-button {}]
             (into [:>
                    RadixSelect/Viewport
                    {:class (merge-classes "p-1"
                             (when (= position "popper")
                               (merge-classes "h-[var(--radix-select-trigger-height)]" "w-full"
...
(defn select-label
  ...
  (into [:>
         ...
         {:data-slot "select-label"
          :class (merge-classes "text-muted-foreground px-2 py-1.5 text-xs" class)}] children))
   ...
   (-> props
    (assoc
     ...
     :class (merge-classes "focus:bg-accent focus:text-accent-foreground"
             ...
             "*:[span]:last:flex *:[span]:last:items-center" "*:[span]:last:gap-2" class))
    (dissoc :class-name))
