...
(defn make-checkbox-column []
  #js {:id "select"
       :header (fn [^js ctx]
                 (let [table (.-table ctx)]
                   [checkbox/checkbox
                    {:checked (or (and (.getIsAllPageRowsSelected table) "all")
...
(defn make-columns []
  #js [(make-checkbox-column)
       #js {:accessorKey "id"
            :header (fn [^js ctx]
                      [sut/column-header-ui
                       {:title "ID"
                        :can-sort? true
            ...
            :cell (fn [^js ctx]
                    (let [priority (.getValue ctx)]
                      [:span {:class (case priority
                       ...
                       priority]))
            :filterFn (fn [row _column-id filter-value]
                        (some #(= % (aget (.-original row) "priority"))
                              (js->clj filter-value)))}
            ...
            :cell (fn [^js _ctx]
                    [dropdown-menu/dropdown-menu {}
                     ...
                     [dropdown-menu/dropdown-menu-content {:align "end"}
                      [dropdown-menu/dropdown-menu-label {} "Actions"]
                      [dropdown-menu/dropdown-menu-item {} "Edit"]
                      [dropdown-menu/dropdown-menu-item {} "Delete"]]])
            :enableSorting false
            :enableHiding false}])

(defscene basic-data-table
  ...
  (let [data (r/atom (clj->js (make-task-data)))
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Tasks"]
        [sut/data-table
         {:columns @columns
          :data @data
          :initial-page-size 5}]]))))

(defscene table-with-toolbar
  ...
  (let [data (r/atom (clj->js (make-task-data)))
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Tasks with Filters"]
        [sut/data-table
         {:columns @columns
          :data @data
...
(defscene table-with-expandable-rows
  ...
  (let [data (r/atom (clj->js (make-task-data)))
        ...
        render-sub-component (fn [row]
                               (let [task (js->clj (.-original row) :keywordize-keys true)]
                                 [:div {:class "p-4 bg-muted"}
                                  [:h4 {:class "font-semibold mb-2"} "Task Details"]
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Expandable Rows"]
...
(defscene table-with-drag-and-drop
  ...
  (let [data (r/atom (make-task-data))
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Drag-and-Drop Reordering"]
        [:p {:class "text-sm text-muted-foreground mb-4"}
...
(defscene table-with-dnd-and-expandable
  ...
  (let [data (r/atom (make-task-data))
        ...
        render-sub-component (fn [row]
                               (let [task (js->clj (.-original row) :keywordize-keys true)]
                                 [:div {:class "p-4 bg-muted"}
                                  [:h4 {:class "font-semibold mb-2"} "Task Details"]
                                  [:div {:class "space-y-2 text-sm"}
                                   [:p [:strong "Full Description: "]
                                    "This is a detailed description of the task. "
                                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit."]
                                   [:p [:strong "Notes: "]
                                    "Additional notes and context about this task."]]]))
        move-row (fn [active-id over-id]
                   (swap! data
                          (fn [tasks]
                            (let [old-index (.findIndex (clj->js tasks)
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Full-Featured Table"]
        [:p {:class "text-sm text-muted-foreground mb-4"}
...
(defscene table-empty-state
  ...
  (let [columns (r/atom (make-columns))
    ...
    (fn []
      (mm-portfolio-utils/wrap-component
       [:div {:class "p-8"}
        [:h3 {:class "text-lg font-semibold mb-4"} "Empty State"]
        [sut/data-table
         {:columns @columns
          :data #js []
