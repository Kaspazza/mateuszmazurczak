           ...
           (fn [{:keys [request-id
             ...
             (bg-worker/init-qr-worker! request-id
                                        ...
                                        {:on-ready on-ready
                                         ...
                                         :on-dispatch (fn [dispatch-v]
                                                        (rf/dispatch dispatch-v))})))

(rf/reg-fx ::request-qr-batch
           (fn [{:keys [request-id]}]
             (bg-worker/request-next-batch! request-id)))

(rf/reg-fx ::cancel-qr-worker
           (fn [{:keys [request-id]}]
             (bg-worker/cancel-request! request-id)))
  ...
  {:qr-codes/on-route-enter
   ...
   (fn [{:keys [db]} [_]]
     (let [page-data (get-in db state-registry/*qr-codes-page-path*)
           {:keys [request-id page-data worker-config]} (qr-processing/start-download page-data)]
   ...
   (fn [{:keys [db]} [_ {:keys [request-id]}]]
     (let [page-data (get-in db state-registry/*qr-codes-page-path*)
           {:keys [page-data request-id]} (qr-processing/handle-worker-ready page-data request-id)]
       {:db (assoc-in db state-registry/*qr-codes-page-path* page-data)
   ...
   (fn [db [_ {:keys [format]}]]
     (let [page-data (get-in db state-registry/*qr-codes-page-path*)
           {:keys [page-data]} (qr-processing/handle-worker-finalizing page-data format)]
   ...
   (fn [{:keys [db]} [_ {:keys [buffer]}]]
     (let [page-data (get-in db state-registry/*qr-codes-page-path*)
       ...
       {:db (assoc-in db state-registry/*qr-codes-page-path* page-data)
        ::save-qr-batch (assoc save-batch
                               :on-success nil
   ...
   (fn [{:keys [db]} [_ errors]]
     (let [page-data (get-in db state-registry/*qr-codes-page-path*)
           {:keys [page-data log-error]} (qr-processing/handle-worker-failure page-data errors)]
   ...
   (fn [{:keys [db]} [_ error]]
     (let [page-data (get-in db state-registry/*qr-codes-page-path*)
           ...
           (qr-processing/handle-download-failure page-data error)
