(ns mateuszmazurczak.ports.events
  "Event dispatching API    
   Frontend is event driven. All side-effects happen through dispatching and realizing events. 
   The complete list of all events is defined in `mateuszmazurczak.adapters.events.registry`
  ...
  (:require
   ...
   [mateuszmazurczak.domain.events.registry :as registry]))
...
(defn dispatch-markers->handlers
