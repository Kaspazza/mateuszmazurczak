(ns mateuszmazurczak.portfolio.application.admin
  (:require
     ...
     [:div {:class "space-y-4"}
      [mm-portfolio-utils/api-component-card
       {:component-name "admin-badge"
        ...
        :props [{:name ":text"      :type "map"      :default nil :description "Must include keys :admin-mode and :admin-logout."}
                {:name ":on-logout" :type "function" :default nil :description "Callback (fn []) triggered on logout click."}]}]
      [mm-portfolio-utils/api-component-card
       {:component-name "delete-solution-button"
        :description "Destructive admin action with confirmation prompt."
        :props [{:name ":solution-id" :type "string | number" :default nil :description "Solution identifier passed to delete callback."}
                {:name ":text"        :type "map"             :default nil :description "Must include keys :confirm-delete and :delete."}
                {:name ":on-delete"   :type "function"        :default nil :description "Callback (fn [solution-id]) after confirmation."}]}]
