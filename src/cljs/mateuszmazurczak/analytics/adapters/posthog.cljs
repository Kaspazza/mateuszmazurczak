...
(defn init!
  ...
  [{:keys [api-key api-host person-profiles] :as config}]
  {:pre [(map? config)
...
(defn capture-event!
  ...
  [{:keys [event properties] :as event-data}]
  {:pre [(map? event-data)
         (validation/valid-non-empty-string? event)]}
  (try
    (if properties
...
(defn identify!
  ...
  [{:keys [user-id properties] :as identity-data}]
  {:pre [(map? identity-data)
         (validation/valid-non-empty-string? user-id)]}
  (try
    (if properties
...
(defn page-view!
  ...
  [{:keys [path properties] :as page-data}]
  {:pre [(map? page-data)]}
  (try
    (let [opts (cond-> {}
