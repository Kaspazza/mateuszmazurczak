    ...
    (cond
      ...
      [:>
       ...
       [ui-sheet/sheet-content
        ...
        (into [:div {:class "flex h-full w-full flex-col"}] children)]]
  ...
  [:button
   (-> props
    ...
    (assoc
     ...
     :class
     (merge-classes
      (str "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear "
      ...
      "[[data-side=right][data-collapsible=offcanvas]_&]:-left-2" class)))])
  ...
  (into
   [:main
    (-> props
     ...
     (assoc :class
      (merge-classes
       (str "relative flex w-full flex-1 flex-col bg-background "
...
(defn sidebar-content
  ...
  (into
   [:div
    (-> props
     (dissoc :class)
     (assoc :data-sidebar "content" :class
      (merge-classes
...
(defn sidebar-group
  ...
  (into [:div
         (-> props
          (dissoc :class)
          (assoc :data-sidebar "group" :class (merge-classes
                                               "relative flex w-full min-w-0 flex-col p-2" class)))]
   children))
    ...
    (into
     [:>
      ...
      (-> props
       ...
       (assoc :data-sidebar "group-label" :class
        (merge-classes (str "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium "
         ...
         "group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0" class)))]
     children)))
...
(defn sidebar-group-action
  ...
  (let [Comp (if as-child Slot "button")]
    (into [:>
           ...
           (-> props
            (dissoc :class :as-child)
            (assoc :data-sidebar "group-action" :class
             (merge-classes
              (str "absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center "
...
(defn sidebar-menu
  ...
  (into [:ul
         (-> props
          (dissoc :class)
          (assoc :data-sidebar "menu" :class (merge-classes "flex w-full min-w-0 flex-col gap-1"
                                              class)))] children))
...
(defn sidebar-menu-item
  ...
  (into [:li
         (-> props
          (dissoc :class)
          (assoc :data-sidebar "menu-item" :class (merge-classes "group/menu-item relative"
                                                   class)))] children))
  ...
  (let [Comp (if as-child Slot "button")
        ...
        (into [:>
               ...
               (-> props
                ...
                (assoc :data-sidebar "menu-button" :data-size (name size) :data-active (boolean
                                                                                        is-active?)
                 :class (merge-classes (menu-button-classes {:variant variant
                                                             :size size}) class)))] children)]
    ...
    (into
     [:>
      ...
      (-> props
       ...
       (assoc :data-sidebar "menu-action" :class
        (merge-classes
         (str
...
(defn sidebar-menu-badge
  ...
  (into
   [:div
    (-> props
     (dissoc :class)
     (assoc :data-sidebar "menu-badge" :class
      (merge-classes
       (str
       ...
       "peer-data-[size=lg]/menu-button:top-2.5" "group-data-[collapsible=icon]:hidden" class)))]
   children))
...
(defn sidebar-menu-sub
  ...
  (into
   [:ul
    (-> props
     (dissoc :class)
     (assoc :data-sidebar "menu-sub" :class
      (merge-classes
       (str
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2.5 py-0.5 ")
       "group-data-[collapsible=icon]:hidden" class)))] children))
    ...
    (into
     [:>
      ...
      (-> props
       ...
       (assoc :data-sidebar "menu-sub-button" :data-size (name size) :data-active (boolean
                                                                                   is-active?)
        :class
        (merge-classes
         (str "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 "
         ...
         size-classes "group-data-[collapsible=icon]:hidden" class)))] children)))
